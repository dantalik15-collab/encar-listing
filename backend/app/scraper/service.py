...
class ScraperService:
    ...
    - Новые объявления → INSERT
    - Существующие (по encar_id) → UPDATE цены и пробега
    """
    ...
    def _convert_price(self, price_krw: int) -> float:
        """Конвертация вон → доллары.

        ENCAR отдаёт цену в 만원 (десятки тысяч вон),
        поэтому умножаем на 10_000 перед конвертацией.
        """
        price_full_krw = price_krw * 10_000
        return round(price_full_krw * settings.krw_to_usd_rate)

    def _map_to_car(self, item: EncarRawItem) -> dict:
        """Маппинг сырых данных ENCAR в поля модели Car."""
        price_full_krw = item.price * 10_000
        return {
            ...
            "price_usd": self._convert_price(item.price),
            "image_url": self._client.build_photo_url(item.photo),
    ...
    async def _upsert_car(self, car_data: dict) -> str:
        """Upsert одного автомобиля. Возвращает 'inserted' или 'updated'."""
    ...
    async def run(self, max_items: int = 100) -> dict:
        ...
        Returns:
            Статистика: количество новых, обновлённых, ошибок.
        """
        logger.info("Запуск парсинга ENCAR", max_items=max_items)

        items = await self._client.fetch_listings(
            max_items=max_items
        )
